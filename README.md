...
* Hides the inner complexity from the user.
* Handles interrupts based on I/O.
* Manages the computer's hardware and I/O devices.
* Memory management.

2.  Describe the job of the Scheduler in the OS in general.

* The Scheduler decides which process runs next, taking into account blocking and non-blocking queues.

3.  Describe the benefits of the MLFQ over a plain Round-Robin scheduler.

* MLFQ executes processes based on priority and time needed to complete, whereas Round Robin scheduler gives each process an equal priority.
