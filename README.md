...
2) Describe the job of the Scheduler in the OS in general.
   ...
   > Scheduler is responsible to orchestrate/manage access to CPU resources. CPU resources are scarce and need to be shared by all the several process an OS can handle. Scheduler is the resource manager, it takes into consideration each process and depending on the implementation it assigns a specific amount of time-access (read it as : resource) to every single process.

3) Describe the benefits of the MLFQ over a plain Round-Robin scheduler.
   > Is about users experience, even at a OS level.
   > CPU resources management is a complex task, all process needs to do its CPU work in order to achieve its purpose, it turns that CPU resources are very scarce. In a simple scenario, with no users expecting any kind of response form the computer, it is possible to put each process to run until completion or to Round_Robin then by the same amount of time-access to the CPU, but we are not in land of machins, we are in land of users interacting with machings, the Scheduler now needs to have that into consideration.
   > MLFQ: Is a way to approach that OS-UX like scenario -> Arm the Scheduler with tools/logic that assign a sort of `priority` to each process, the ones that a user demands a faster response form will be the ones the Scheduler will assign more time-access to CPU resources (that sort of management makes happy users and efficient multi-process systems)
