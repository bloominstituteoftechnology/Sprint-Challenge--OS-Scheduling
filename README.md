...
1.  Name at least three things that a general-purpose operating system is responsible for handling.
    a. Kernel: the program that runs the CPU
    b. Processes: applications that runs in user space
    c. Threads: processes which share address space

2.  Describe the job of the Scheduler in the OS in general.
    The scheduler manages process execution time and order. This is what make computers
    seemingly able to multitask.

3.  Describe the benefits of the MLFQ over a plain Round-Robin scheduler.
    With a MLFQ, processes are assigned priority levels and are executed in order of, high
    priority/short runtime, high priority/long runtime, low priority/short runtime and low
    priority/long runtime. This results in a more efficient and smooth running OS.
