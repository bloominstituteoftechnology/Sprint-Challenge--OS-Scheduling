...
1.  Name at least three things that a general-purpose operating system is responsible for handling.
    User input, output. Memory allocation. Intermediary between programs and hardware.

2.  Describe the job of the Scheduler in the OS in general.
    it selects processes that are currently in queue and loads them into memory so that they can be run.

3.  Describe the benefits of the MLFQ over a plain Round-Robin scheduler.
    If you know that a particular process will take longer than others, you can give it a higher 'weight' so that the scheduler knows to handle it differently.
