...
* I think Program Execution, I/O Operations, File-System Manipulation, Communications, and Error Detection.

2.  Describe the job of the Scheduler in the OS in general.

* The Scheduler carries out the scheduling activities through computational elements like threads, processes, etc. Schedulers are implemented to keep the load balanced and allow users (multiple) to share those resources.

3.  Describe the benefits of the MLFQ over a plain Round-Robin scheduler.

* This processing scheme continues until it is complete or when it reaches its base level queue. This can also be on a first come first served basis in the Round-Robin fashion.
