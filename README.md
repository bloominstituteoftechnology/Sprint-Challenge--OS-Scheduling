...
The operating system is responsible for managing between the hardware's resources which will 
include the processors, memory, the data storage, and I/O devices. Also hiding the complexities of hardware from the user, along with handling interrupts generated by the I/O controllers. 

2. Describe the job of the Scheduler in the OS in general. 
The job of the Scheduler in the OS in general is it selects processes from the queue and loads them into the memory to execute them. Process goes into the memory for the CPU scheduling. The main job is to get all the jobs done and control the whole programming process. 
