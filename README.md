...
reads inputs from users, displays outputs on monitors, allows applications to run
2. Describe the job of the Scheduler in the OS in general.
allocates resources for methods to be run
3. Describe the benefits of the MLFQ over a plain Round-Robin scheduler.
round robin will increase the average quantum time for processes and not give enough priority to interactive/important processes
