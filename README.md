...
1.  Name at least three things that a general-purpose operating system is responsible for handling.
    -The OS manages interactions between programs and the hardware inside my computer.
    -The OS manages the input and output to and from your hardware
    -The OS provides a way of allowing you to interact with your device and other programs

2.  Describe the job of the Scheduler in the OS in general.
    -When more than one process is runable, the OS decides which one is first through the scheduler.

3.  Describe the benefits of the MLFQ over a plain Round-Robin scheduler.
    -With MLFQ, processes are assigned priority based on past behaviour. Is it I/O bound or CPU bound?
