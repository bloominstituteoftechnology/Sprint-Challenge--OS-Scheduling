...
1.  Name at least three things that a general-purpose operating system is responsible for handling.
    ...
    * A general purpose operating system is responsible for handling drivers, memory management, process scheduling, and controlling access to core systems.

2.  Describe the job of the Scheduler in the OS in general.

    * The scheduler decides which processes to run, taking into consideration relavent variables such as the importance of the process and the amount of time a process has already had to run.

3.  Describe the benefits of the MLFQ over a plain Round-Robin scheduler.

    * A Round-Robin is a little more sophisticated than a simple Queue, in that it allocates a standard amount of time for each process, starting at the top of the Queue, and working downwards. The MLFQ is even more sophisticated in that it increases the amount of time allocated for processes that have previously been given time (aka they were not able to complete within the first allocated amount of time). There is also a higher priority level called blocking, but I don't really understand that piece of MLFQ.
