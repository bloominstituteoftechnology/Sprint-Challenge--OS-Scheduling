...
1.  Name at least three things that a general-purpose operating system is responsible for handling.
    * Interfaces with the hardware on a computer.
    * Handles Processes on computer.
    * Scheduling processes.
2.  Describe the job of the Scheduler in the OS in general.
    * It organizes processes to start or end in an efficient manner.
3.  Describe the benefits of the MLFQ over a plain Round-Robin scheduler.
    * It gives higher priority processes less time and more power to boot up while it leaves the lesser processes to start in a slower less demanding order.
