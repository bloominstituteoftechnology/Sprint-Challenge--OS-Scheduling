...
* The scheduler is responsible for controlling when and how processes are managed and executed. Different schedulers have different strategies for handling multiple queued processes. They all usually involve doing one process for a short amount of time, before switching to another, and looping through the processes indefinitely. The scheduler is in charge of removing processes that have completed from the queue.
