...
2.  Describe the job of the Scheduler in the OS in general.
    ...
    The scheduler works sort of like a load balancer in attempting to complete jobs in an efficient manner by giving priority to processes based on time to completion.

3.  Describe the benefits of the MLFQ over a plain Round-Robin scheduler.

    MLFQ scheduler is a "smart" system in that it will identify and give higher priority to processes that require less power and take less time to complete, thus elimiinating jobs from the queue quicker. Slower, more power hungry processes have their priority levels systematically lowered as CPU resource threshold is balanced to produce more efficient scheduling.
