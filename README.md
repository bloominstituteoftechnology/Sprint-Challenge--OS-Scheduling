...
An operating system is responsible for handling interactions between programs/external output and hardware, organization of files, and schedules when processes should be run.

2.  Describe the job of the Scheduler in the OS in general.

The scheduler ensures that processes run efficiently and fluidly, so that long processes do not block small periodic processes from running nearly instantaneously. By having a scheduler, many processes can be "running" simultaneously even though the CPU has to read instructions one at a time because the scheduler gives each process a short time to run and then stops it even if it isn't finished in order to give some time for the next process it wants to run.

3.  Describe the benefits of the MLFQ over a plain Round-Robin scheduler.

A Round-Robin scheduler would make processes run longer than they have to by not prioritizing. Using multiple queues, a MLFQ can put processes into different priorities based on how long the process has run so far, thus finishing short processes the fastest.
